...
public class RequestDataController {
    ...
    // GET 요청 파라미터 받는 방법 4 -    해당 파라미터를 필수 할목으로 설정할수 있다. (기본 값 True)
    // 정수 자료형의 파라미터가 필수가 아닐 때 Integer 자료형을 사용해야한다. (null 처리 가능하기 때문)
    @GetMapping("/req/data4")
    public ResponseEntity<Map<String, String>> reqGet4(String name, @RequestParam(required = false) Integer age) {
    ...
    public ResponseEntity<Map<String, String>> reqGet6(ReqDataDto6 dto) {
        System.out.println("data5: " + dto.getName());
        System.out.println("data5: " + dto.getAge());
        return ResponseEntity.ok().build();
    }
